#!/usr/bin/env bash
# Launch the Blackwell xformers environment setup on capture PCs.
#
# This intentionally opens at most one SSH session per PC. Progress is written
# to the shared log root so the robot PC can monitor without repeated SSH.
set -euo pipefail
...
LOG_ID="${LOG_ID:-gotrack_sm120_$(date +%Y%m%d_%H%M%S)}"
...
status() {
    echo "[env-setup] log root: $LOCAL_LOG_ROOT"
...
launch() {
    mkdir -p "$LOCAL_LOG_ROOT"
    echo "[env-setup] launching setup on: ${PCS_LIST[*]}"
    echo "[env-setup] local log root: $LOCAL_LOG_ROOT"
    local last_index=$((${#PCS_LIST[@]} - 1))
    for i in "${!PCS_LIST[@]}"; do
        pc="${PCS_LIST[$i]}"
        echo "[env-setup] launching $pc"
        remote_cmd=$(cat <<EOF
set -eu
REPO=$REMOTE_REPO
