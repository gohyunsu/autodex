...
- `scripts/run_gotrack_episode_scheduler.py`
  ...
  - 한 번 launch되면 각 worker는 남은 claimable episode가 없을 때까지 자동으로
    `claim -> GoTrack -> overlay -> output 검증 -> 다음 claim`을 반복한다.
    Codex나 수동 SSH 세션이 계속 붙어 있을 필요는 없다.
...
- `scripts/gotrack_episode_dashboard.py`
  ...
  - dashboard는 `<schedule_dir>`의 manifest/task/worker/event 파일만 읽는
    file-based viewer라서 worker가 모두 종료된 뒤에도 같은 schedule directory로
    다시 실행해 완료 상태와 overlay 영상을 확인할 수 있다.
...
- `Stage Flow`: episode lifecycle을 stage별로 설명하고 현재 분포를 숫자로
  보여준다. stage는 `Pending -> Claimed -> Starting -> GoTrack -> Overlay ->
  Complete` 순서로 진행되며, 실패 시 `Failed`, dry-run 실행 시 `Dry Run`으로
  표시된다.
...
- `Episode Work Queue`: episode별 상태, object, episode id, stage,
  ...
  episode에 `overlay_*.mp4`가 있으면 대표 overlay thumbnail 하나를 표시한다.
  thumbnail 양쪽의 `<`/`>` control로 같은 episode의 다른 camera overlay로
  넘길 수 있고, thumbnail을 누르면 같은 화면의 player에서 바로 재생된다. player
  안에서도 이전/다음 overlay video로 넘길 수 있다. thumbnail은
  `/overlay-thumb?path=...` endpoint가 첫 요청 때 JPEG로 생성해
  `<schedule_dir>/.overlay_thumbs/`에 cache한다.
...
- `Pending`: queue에 있지만 아직 어떤 PC도 claim하지 않은 episode.
- `Claimed`: worker가 atomic lock directory를 만들고 실행 준비 중인 episode.
- `Starting`: subprocess는 시작됐지만 GoTrack/overlay progress line이 아직
  나오지 않은 초기 구간.
- `GoTrack`: 한 episode의 모든 camera video를 같이 읽어 multi-view tracking을
  수행하고 `world_pose_records.json`을 만드는 구간.
- `Overlay`: tracking 결과를 사용해 camera별 `overlay_<serial>.mp4`를 렌더링하는
  구간.
- `Complete`: 필요한 output이 이미 있었거나 새로 생성되어 재실행 대상에서
  제외된 상태.
- `Failed`: subprocess error 또는 output 검증 실패. `--retry-failed`를 줘야
  다시 claim 대상이 된다.

이미 실행된 episode 판단:

- schedule 생성 시 각 episode마다
  `<episode>/object_tracking/gotrack_output/world_pose_records.json`을 읽어
  하나 이상의 `pose_world` record가 있으면 GoTrack 완료로 본다.
- overlay stage가 필요한 schedule에서는
  `/home/robot/shared_data/AutoDex/object_overlay_video/<hand>/<obj>/<episode>/overlay_<serial>.mp4`
  파일이 expected camera serial 전체에 대해 존재해야 overlay 완료로 본다.
- `--stages both`에서는 GoTrack과 overlay가 모두 완료된 episode를
  `skipped_done`으로 기록하고 worker가 claim하지 않는다.
  `--stages gotrack` 또는 `--stages overlay`에서는 해당 stage의 완료 여부만으로
  skip을 판단한다.
...
빌드하고, GoTrack subprocess의 xformers attention 호출만 BF16으로 변환하는
wrapper 환경을 맞춘다.
...
- `scripts/launch_object_overlay_env_setup.sh`
  - robot PC에서 실행한다.
  - GoTrack xformers setup launcher와 같은 방식으로 `capture1`, `capture2`,
    `capture3`, `capture5`, `capture6`에 SSH를 PC당 한 번만 열어 overlay env
    setup을 background로 시작한다.
  - 이후 상태 확인은 SSH 반복 접속이 아니라 공유 로그 파일을 읽는다.
...
GoTrack env는 `[verify] memory_efficient_attention_bf16_ok`와 `[setup] done`,
overlay env는 `overlay_imports_ok`와 `[overlay-env] done`이 각 PC setup log에
있어야 해당 PC 환경 재설정이 완료된 것이다.
...
5개 capture PC에 overlay env setup도 한 번씩만 SSH로 launch:

```bash
LOG_ID=overlay_env_20260701 \
STAGGER_SECONDS=60 \
bash scripts/launch_object_overlay_env_setup.sh launch
```

overlay env 진행 상태 확인:

```bash
LOG_ID=overlay_env_20260701 \
bash scripts/launch_object_overlay_env_setup.sh status
```
...
overlay env도 동일하게 실패한 PC만 다시 대상으로 잡는다.
